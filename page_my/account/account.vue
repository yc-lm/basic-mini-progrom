<template>
  <s-layout title="姓名" :bgStyle="{ color: '#FFFFFF' }">
    <view class="p-16">
      <uni-forms ref="baseForm" :rules="state.rules" validateTrigger="bind">
        <uni-forms-item name="name">
          <uni-easyinput
            v-model.trim="state.userInfo.nickname"
            placeholder="请输入姓名"
            :inputBorder="false"
          />
        </uni-forms-item>
...
<script setup>
  ...
  import { realName } from '@/sheep/validate/form';
  import sheep from '@/sheep';
  import { onShow } from '@dcloudio/uni-app';
  import { OK } from '@/sheep/hooks/useApi';
  ...
  async function submit() {
    unref(baseForm)
      .validate(async (err, formData) => {
        if (!err) {
          const { error } = await sheep.$api.user.editUserInfo({
