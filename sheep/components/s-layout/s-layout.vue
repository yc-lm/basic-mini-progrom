  ...
  >
    <view class="page-main" :style="[bgMain]">
      ...
      <su-navbar
        ...
        :isMore="isMore"
        @clickLeft="emits('clickLeft')"
        :is-click-left="isClickLeft"
      />
    ...
    <!--   课堂提醒-->
    <s-course-modal></s-course-modal>
    <!--全局提示  解决与loading 冲突-->
    <s-popup-toast></s-popup-toast>
  </view>
</template>
...
<script setup>
  ...
  const emits = defineEmits(['search', 'clickLeft']);
...
<style lang="scss" scoped>
  .page-app {
    position: relative;
    color: var(--ui-TC-TITLE);
    background-color: var(--ui-BG-Main-WHITE) !important;
    z-index: 2;
    display: flex;
    width: 100%;
    height: 100vh;
