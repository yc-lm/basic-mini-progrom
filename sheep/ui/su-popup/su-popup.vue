  ...
  >
    <view @touchstart="touchstart">
      ...
      >
        ...
        >
          <!--为了支持头部显示标题-->
          <view class="popup-header relative" v-if="isShowHeader">
            <text class="header-title absolute absolute-center f-s-17 f-w-600 col-b-title">{{
              title
            }}</text>
            <view class="header-close absolute h-100p flex align-center justify-center">
              <uni-icons
                v-if="showClose"
                class="close-icon col-b-title-6"
                type="closeempty"
                size="16"
                @click="close"
              ></uni-icons>
  ...
  export default {
    ...
    methods: {
      ...
      bottom(type) {
        ...
        console.log('bottom', this.transClass)
        // TODO 兼容 type 属性 ，后续会废弃
        if (type) return;
        this.showPopup = true;
        this.showTrans = true;
...
<style lang="scss">
  .uni-popup {
    ...
    .uni-popup__wrapper {
      ...
      .popup-header {
        height: 48px;
        border-bottom: 1px solid rgba(26, 29, 52, 0.1);
